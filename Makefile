...
NODE_TAGS := with_quic with_utls with_wireguard with_acme with_clash_api
REQUIRED_NODE_TAGS := with_utls

.PHONY: build clean test docker install build-linux build-linux-arm64 build-all

# Build for current platform
build:
	go build -ldflags "$(LDFLAGS)" -tags "$(NODE_TAGS)" -o xboard-node ./cmd/xboard-node
	sh scripts/check-node-build-tags.sh xboard-node $(REQUIRED_NODE_TAGS)
	go build -ldflags "$(LDFLAGS)" -o xbctl ./cmd/xbctl

# Build for Linux amd64
build-linux:
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags "$(LDFLAGS)" -tags "$(NODE_TAGS)" -o xboard-node-linux-amd64 ./cmd/xboard-node
	sh scripts/check-node-build-tags.sh xboard-node-linux-amd64 $(REQUIRED_NODE_TAGS)
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -ldflags "$(LDFLAGS)" -o xbctl-linux-amd64 ./cmd/xbctl

# Build for Linux arm64
build-linux-arm64:
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 go build -ldflags "$(LDFLAGS)" -tags "$(NODE_TAGS)" -o xboard-node-linux-arm64 ./cmd/xboard-node
	sh scripts/check-node-build-tags.sh xboard-node-linux-arm64 $(REQUIRED_NODE_TAGS)
